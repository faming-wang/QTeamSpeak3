...
#include "private/definitions_p.h"
...
public:
    ...
    ~InterfaceManager();
    static void setLibrary(QLibrary* lib);
    ...
    uint64 spawnNewServerConnectionHandler(int port);
    ...
    bool openPlaybackDevice(const Connection* server, const ISoundDevice* playbackDevice = Q_NULLPTR);
    bool openCaptureDevice(const Connection* server, const ISoundDevice* captureDevice = Q_NULLPTR);

    QString getCurrentPlaybackDeviceName(const Connection* server, bool* isDefault = Q_NULLPTR);
    QString getCurrentPlayBackMode(const Connection* server);

    QString getCurrentCaptureDeviceName(const Connection* server, bool* isDefault = Q_NULLPTR);
    ...
    // logging
    bool logMessage(const QString& logMessage, LogLevel severity, const QString& channel, const Connection* server = Q_NULLPTR);
    ...
    // ReturnCode requestSlotsFromProvisioningServer(const QString& ip, ushort port, const QString& serverPassword, ushort slots, const QString& identity, const QString& region, uint64* requestHandle);
    // ReturnCode cancelRequestSlotsFromProvisioningServer(uint64 requestHandle);
    ...
    uint64 getConnectionVariableAsUInt64(const Client* client, ConnectionProperty flag);
    ...
    // uint64 getServerConnectionVariableAsUInt64(const Connection* server, size_t flag);
    ...
    uint64 getClientVariableAsUInt64(const Client* client, ClientProperty flag);
    ...
    uint64 getChannelVariableAsUInt64(const Channel* channel, ChannelProperty flag);
    QString getChannelVariableAsString(const Channel* channel, ChannelProperty flag);
    // uint64 getChannelsFromChannelNames(const Connection* server, const QStringList& channelNames);
    bool setChannelVariableAsInt(const Channel* channel, ChannelProperty flag, int value);
    bool setChannelVariableAsUInt64(const Channel* channel, ChannelProperty flag, uint64 value);
    ...
    uint64 getServerVariableAsUInt64(const Connection* server, VirtualServerProperty flag);
    ...
    uint64 getTransferFileSize(const FileTransfer* transfer);
    uint64 getTransferFileSizeDone(const FileTransfer* transfer);
    ...
    uint64 getTransferRunTime(const FileTransfer* transfer);
    ...
    uint64 getInstanceSpeedLimitUp();
    uint64 getInstanceSpeedLimitDown();
    uint64 getServerConnectionHandlerSpeedLimitUp(const Connection* server);
    uint64 getServerConnectionHandlerSpeedLimitDown(const Connection* server);
    uint64 getTransferSpeedLimit(const FileTransfer* transfer);
    bool setInstanceSpeedLimitUp(uint64 newLimit);
    bool setInstanceSpeedLimitDown(uint64 newLimit);
    bool setServerConnectionHandlerSpeedLimitUp(const Connection* server, uint64 newLimit);
    bool setServerConnectionHandlerSpeedLimitDown(const Connection* server, uint64 newLimit);
    bool setTransferSpeedLimit(const FileTransfer* transfer, uint64 newLimit);

private:
    InterfaceManager(QObject* parent = Q_NULLPTR);
    friend void __init_interface_manager();
};
